...
class StateSuite extends FunSuite {
  ...
  test("State.sequence_r should return correct combinator")
  {
    val rng = new ConstantRNG(-1)

    val d3Rand = State.sequence_r(List[State.Rand[Int]](State(RNG.nonNegativeInt), State(RNG.int), State(RNG.int)))
